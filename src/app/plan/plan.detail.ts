...
@Component({
  ...
  templateUrl : './_view/plan.detail.html',
  providers: [ ]
})

export class PlanDetailForm implements OnInit, OnDestroy { 

    response : Response;  
    cPracoviste: number = -1;
    cZdroj: number = -1;
    cKalendar: number = -1;
    lstPracoviste: Pracoviste[] = [];
    lstZdroj: Zdroj[] = [];
    plan: Plan;
    ...
    ngOnInit(): void { 
        ...
        this.cisService.getPracovistePlanovani().then(( prac: Pracoviste[]) => {
             this.lstPracoviste = prac;
             ...
             this.route.params.pipe(switchMap((params: Params) => { 
                ...
                for (var i = 0; i < this.lstPracoviste.length; ++i){
                    
                    if (this.lstPracoviste[i].id == id){
                        this.cPracoviste = i;
                        isfound = true;
                        break;
                    }
    ...
    refreshView(){
       ...
       jQuery(function(){           
            ...
            jQuery('#dt-picker-prac').datetimepicker({
              locale: 'cs',
              format: 'YYYY-MM-DD',
              inline: true
   ...
   loadZdroje(){       
       ...
       this.cisService.getZdrojList(this.lstPracoviste[this.cPracoviste]).then((lst: Zdroj[]) => {
            this.lstZdroj = lst;
   ...
   markCalendar(){
       ...
       jQuery('#dt-picker-prac td').removeClass('marked'); 
       ...
       this.planService.getZaplanovaneDny(this.lstZdroj[this.cZdroj], mesic, rok).then((response:Response)=> {
           ...
           for(var i=0; i< response.data.length; ++i){
                ...
                x = '#dt-picker-prac td[data-day="'+response.data[i]+'"]';             
